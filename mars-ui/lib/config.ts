 ...
 *
 * In the browser: always use window.location.host (same host+port the user
 * typed in the address bar). This routes through NGINX which proxies /ws
 * to the backend — no hardcoded IPs, no localhost leaks, no port exposure.
 *
 * Server-side (SSR): use the configured WS URL so the server can reach the
 * backend directly.
 */
